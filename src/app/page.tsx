...
export default function Home() {
  ...
  // Always start at index 0 so server and client agree on first render
  ...
  // Stable ref to the tick function so interaction handlers can restart without duplication
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setTimeout(() => {
        setTaglineIndex(prev => {
          let next
          do { next = Math.floor(Math.random() * taglines.length) } while (next === prev)
          return next
        })
        setTaglineOpacity(1)
      }, 600)
    ...
    taglineIntervalRef.current = setInterval(tick, 60000)
    return () => {
      if (taglineIntervalRef.current) clearInterval(taglineIntervalRef.current)
    }
  }, [mounted])

  const handleCalcInteractionStart = () => {
    if (taglineIntervalRef.current) {
      clearInterval(taglineIntervalRef.current)
      taglineIntervalRef.current = null
    }
  }

  const handleCalcInteractionEnd = () => {
    if (taglineIntervalRef.current) clearInterval(taglineIntervalRef.current)
    taglineIntervalRef.current = setInterval(tickRef.current, 60000)
  }
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#1A1410' }}>
      ...
      <section style={{
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2.8rem, 6vw, 5.5rem)', fontWeight: 900, lineHeight: 1.1, color: '#F5F0E8', maxWidth: '800px', marginBottom: '24px', opacity: mounted ? taglineOpacity : 1, transition: mounted ? 'opacity 0.6s' : 'none' }}>
        ...
        <div style={{ width: '100%', maxWidth: '680px', marginTop: '80px' }}>
          <ProfitCalculator
            onInteractionStart={handleCalcInteractionStart}
            onInteractionEnd={handleCalcInteractionEnd}
          />
        </div>
      </section>
